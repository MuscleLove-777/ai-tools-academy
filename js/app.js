/* ============================================
   AI Tools Academy - Main Application
   ============================================ */
...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('ai-tools-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('ai-tools-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('ai-tools-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('ai-tools-progress', JSON.stringify(this.progress));
        localStorage.setItem('ai-tools-quiz-results', JSON.stringify(this.quizResults));
    },
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero">
                    <h2>AI別特徴・活用例アカデミー へようこそ</h2>
                    <p>主要AIサービスの特徴と活用法を体系的に学びましょう。テキスト・画像・音声・動画・コーディングなど全カテゴリのAIを網羅します。</p>
                </div>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('ai-tools-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('ai-tools-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('ai-tools-progress');
            localStorage.removeItem('ai-tools-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
